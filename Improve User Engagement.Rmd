...
dat.demo <- read_csv("Data/emographics.csv", col_names = TRUE)
...
There are 3811 total unique users in  app_usage and demographic data sets. There are only 1865 unique users in the ex survey data sets. In the ex survey data, only 935 users completed the survey twice (immediately after signing up and 14 days post); 930 users only completed it once.
```{r}
...
### App Usage Trend over 14 days
Total App Usage per day for 14 days (before imputation for missing data; averaged by total number of users)
```{r}
 dat.appuse.demo %>% 
...
dat.appuse.demo %>% 
  ...
  scale_x_continuous(  breaks = c(1:14,1)) +
    theme_classic()
```
Total App Usage per day for 14 days (before imputation for missing data; averaged by total available data points per day)
```{r}
...
```
For each metro area, there's a downward trend of total app usage (sum of app usage, averaged by the number of users in each area) over the span of 14 days. Missing data in days 10-14 are not imputed here.
```{r}
dat.appuse.demo %>% 
  group_by(metro_area) %>%
  summarise_at(vars(day1:day14),
...
#### metro_area
Metro areas with top 5 user sizes: LA, SF, None of Above, DS, SEA, NYC
```{r}
...
dat.appuse.demo %>% 
  ...
  coord_flip() 
```
There's not much difference in terms of total usage over 14 days (averaged by the numbers of users) in different metro areas
```{r}
dat.appuse.demo %>% 
  group_by(metro_area) %>%
...
#### age_range
core users are within 25 to 34 age range. Not much differences in terms of average total use
```{r}
...
First, look at descriptive statistics: there's a slight decrease in stress level after using the app for 14 days
```{r}
glimpse(dat.userex)
...
Correlations: look at whether users who expected the app to be helpful right signing up for the app would report they found the app to be helpful after 14 days (for users who completed the survey at both times) --> no correlations
```{r}
...
In dat.appuse file, NAs mean the app lost connection with this user’s phone due to a technical error, and do not have usage data for these individuals on those days. So missing values are MCAR or MAR and need to be addressed accordingly.--> use multiple imputation
```{r}
...
#                     ))))
```
Age and Gender drives usage: Age (18 to 24) has higher usage compared to 25-34 (but not different from other age ranges); Males have higher usage compared to female (other genders do not different from female)
```{r}
...
Users' latent profiles based on their sign-up surveys do not predict total usage
```{r}
summary(lm(totalappuse~., data = dat.appuse.demo.MIcompleted.userex_signup_P4 %>% 
             select(totalappuse, Class)))
```

correlations between 6 items and totalappuse: sign-up items do not correlate with totalappuse
```{r}
...
Item 3 (I am able to focus on the present moment) and Item 2 (It is easy for me to concentrate on what I'm doing) (nearly significantly) predict total usage
```{r}
summary(lm(totalappuse~., data = dat.appuse.demo.MIcompleted.userex_signup_P4 %>% 
...
```
Item 2 ( "It is easy for me to concentrate on what I'm doing.") explains 2nd largest variance in total usage.
```{r}
library(rwa)
